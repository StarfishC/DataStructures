...
### 1.8 阅读下列算法，指出算法 A 的功能和时间复杂度，其中 h,g 分别是单循环链表中的两个节点指针(教材已有)
...
### 1.10 设 n 是偶数，计算运行下列程序段后 m 的值并给出该程序段的时间复杂度(教材已有)
...
### 1.11 有下列运行时间函数, 分别写出相应的大$O$表示的运算时间（教材由）
