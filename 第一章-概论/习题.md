...
> **解答：**
>
> - 当i=1时，第二重循环执行n次，第三重循环执行n次  
> 当i=2时，第二重循环执行n-1次，第三重循环执行n-1次  
> 当i=n-1时，第二重循环执行2次，第三重循环执行2次  
> 所以$f(n)=\sum_{i=1}^{i=n-1}2(n-i+1)=(n+2)(n-1)$
> - 当n=5时，$f(n)=28$
...
> **解答：**
> 外层循环执行n次，有效次数是n/2，1次赋值操作
>
> 当i=1时，j循环n/2；当i=2时，j循环n/4，所以可得m执行次数为：
...
> $$
> 所以m的值为$\frac{n}{2}\ln{(\frac{n}{2}+1)}+\frac{nC}{2}$，时间复杂度为$O(n\ln{n})$
...
> **解答：**
>
> (1)时间复杂度是$O(n)$, 一共需要赋值操作1次$i$和$n$次x，共$n+1$次
>
> (2)时间复杂度是$O(n^2)$, 一共需要赋值操作$n^2$次x，n次$j$和1次$i$，一共是$n^2+n+1$次
