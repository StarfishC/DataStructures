...
2. 树的链式存储
   ...
   - 动态表示法  
      由于树的结点可以有任意的度数，并且各结点的子结点树随时发生变化。树结点的动态性给树的实现带来困难。可为每个结点分配可变的存储空间，每一个结点都存储一个子结点指针表，其子结点的数目也存储在该节点中。该表示法本质上与“子结点表”表示法相同，但是它可以动态地分配节点空间，而不是把所有节点分配在同一个数组中
...
3. 扩充二叉树（$extended \; binary \; tree$）  
   ...
   构造一棵扩充二叉树只需要在原二叉树树叶下增加两个新的空树叶，度数为$1$的分支节点下增加一个空树叶即可。扩充二叉树是满二叉树，新增空树叶（外部节点）的个数等于原二叉树结点（内部结点）个数加$1$
   ...
   > 设$n$个内部结点时成立，有$E_n=I_n+2n$。当有$n+1$个内部结点时，如果删除一个分支节点，该分支节点到根结点的路径长度是$K$，可使之成为有$n$个结点的扩充二叉树。由于删除了一个路径长度为$K$的内部结点，因此内部节点路径长度变为$I_n=I_{n+1}-K$，由于减少了两个路径长度为$K+1$的外部结点，增加了一个路径长度为$K$的外部节点，外部路径长度变为$E_n=E_{n+1}-2(K+1)+K=E_{n+1}-K-2$，由此可推出$E_{n+1}=I_n+2n+K+2=I_{n+1}+2(n+1)$。证毕
...
4. 满二叉树定理：非空满二叉树树叶数目等于其分支节点树加$1$
...
7. 对于具有$n$个结点的完全二叉树，结点按层次由左到右编号，则对任一结点$i(i≤n-1)$有：
   ...
   - （2）当$2i+1≤n-1$时，结点$i$的左子节点是$2i+1$，否则结点$i$没有左子节点  
     当$2i+2≤n-1$时，结点$i$的右子节点是$2i+2$，否则结点$i$没有右子节点
...
>    对于$i=0$，由完全二叉树的定义，其左边孩子的编号是$1$，如果$1>n-1$，即不存在编号为$1$的结点，此时结点$i$没有左子节点。其右子结点的编号只能为$2$，如果$2>n-1$，此时结点$i$没有右子结点  
>    对于$i>0$分两种情况：  
>    ① 设第$j$层的第一个节点编号为$i$（此时有$i=2^j-1$），则其左子节点必为第$j+1$层的第一个结点，其编号为$2^{j+1}-1=2i+1$。如果$2i+1>n-1$，那么$i$没有左子节点；其右子结点必为第$j+1$层第二个结点，其编号为$2i+2$  
>    ② 假设第$j$层的某个节点编号为$i$，若它有左子节点，那么它的左子节点必然是第$j+1$层中的第$2[i-(2^j-1)]+1$个，其左子结点的编号为$(2^{j+1}-1)+2[i-(2^j-10)]=2i+1$；如果结点$i$有右子结点，那么其右子结点的编号必定是$2i+2$
...
2. 二叉树 -> 树或森林  
   ...
   - 旋转：以根为轴，平面逆时针方向旋转
   - 补线：若结点$x$是父结点$y$的左子节点，则把$x$的右子结点、右子结点的右子结点，依次类推，直到最右子结点，用连线与$y$连接起来
   - 删线：去掉所有父结点到右子结点的连线
